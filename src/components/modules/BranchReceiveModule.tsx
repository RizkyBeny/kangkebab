...
export const BranchReceiveModule: React.FC<BranchReceiveModuleProps> = ({
  ...
  const handleConfirmReception = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Gagal mengonfirmasi penerimaan barang');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 md:space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-slate-900 tracking-tight flex items-start gap-2 leading-tight">
            <PackageCheck className="w-6 h-6 text-slate-700 flex-shrink-0 mt-0.5 md:mt-1" />
        ...
        <Button onClick={onRefresh} variant="outline" className="h-11 md:h-10 text-xs md:text-sm font-bold shadow-sm bg-white w-full sm:w-auto">
      ...
      <div className="space-y-4">
        ...
        {pendingShipments.length === 0 ? (
          <Card className="p-8 text-center space-y-2 border-slate-200 shadow-sm border-dashed rounded-xl">
            ...
            <p className="text-sm font-bold text-slate-900">Tidak Ada Pengiriman Menunggu</p>
                ...
                <CardContent className="p-5 space-y-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <h4 className="text-lg font-mono font-bold text-slate-900 mt-2">{s.shipmentNumber}</h4>
                  ...
                  <div className="bg-slate-50 rounded-xl p-3 border border-slate-200 space-y-1 text-xs">
                    <div className="font-bold text-slate-800 mb-2 flex justify-between">
                      ...
                      <span className="text-slate-500 font-medium">{s.items.length} jenis</span>
                    </div>
                    {s.items.map((item) => (
                      <div key={item.id} className="flex items-center justify-between text-slate-700 font-medium">
                        ...
                        <span className="font-bold text-slate-900 whitespace-nowrap">{item.qtySent} unit</span>
                      </div>
                    ))}
                  </div>
      ...
      <div className="space-y-4 pt-6 border-t border-slate-200">
          ...
          <Table>
            <TableHeader className="bg-slate-50/80 border-b border-slate-100">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Waktu Diterima</TableHead>
                <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Status</TableHead>
                <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider text-right">Rincian</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {completedShipments.length === 0 ? (
                ...
                completedShipments.map((s) => (
                  <TableRow key={s.id} className="group transition-colors hover:bg-slate-50/50">
                    ...
                    <TableCell className="text-slate-600 text-sm font-medium">{s.receivedAt ? formatDate(s.receivedAt) : '-'}</TableCell>
                    <TableCell>
        ...
        <div className="md:hidden space-y-4">
          {completedShipments.length === 0 ? (
            <Card className="p-8 text-center text-slate-400 text-sm border-slate-200 rounded-xl">
              Belum ada pengiriman yang dikonfirmasi.
            </Card>
          ) : (
            completedShipments.map((s) => (
              <Card key={s.id} className="border-slate-200 shadow-sm rounded-xl overflow-hidden">
                <CardContent className="p-4 space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="font-mono font-bold text-slate-900 text-base">{s.shipmentNumber}</div>
      ...
      >
        {selectedShipment && (
          <form onSubmit={handleConfirmReception} className="space-y-4">
            <div className="flex flex-col items-center justify-center p-3 bg-slate-50 rounded-xl border border-slate-100 mb-2">
              <span className="font-mono font-bold text-base text-slate-900">{selectedShipment.shipmentNumber}</span>
              <span className="text-xs text-slate-500 mt-0.5">Dikirim: {formatDate(selectedShipment.sentAt)}</span>
            </div>
            ...
            <div className="space-y-3">
              <div className="text-xs font-bold text-slate-900">Cek Kondisi Fisik Barang:</div>
              ...
              <div className="space-y-3 max-h-[45vh] overflow-y-auto pr-1">
                {selectedShipment.items.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3 shadow-sm">
                      <div className="flex items-start justify-between">
                        <div>
                          <div className="font-bold text-slate-900 text-sm">{item.masterProduct.name}</div>
                          <div className="text-[11px] text-slate-500">Varian: {item.masterProduct.variant}</div>
                        </div>
                        <div className="text-right">
                          <span className="text-[10px] uppercase text-slate-400 font-medium">Target HQ:</span>
                          <div className="font-bold text-slate-900 text-sm">{item.qtySent} unit</div>
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-200">
                        <div className="space-y-1.5">
                          <Label className="text-[11px] font-semibold text-slate-700">Total Fisik Diterima</Label>
                          <Input
                            type="number"
                            min={0}
                            value={conf.qtyReceived}
                            onChange={(e) => updateQuantity(item.id, 'qtyReceived', Number(e.target.value))}
                            className="h-11 md:h-10 text-sm md:text-xs font-mono text-center bg-white"
                          />
                        </div>
                        <div className="space-y-1.5">
                          ...
                          <Input
                            ...
                            className="h-11 md:h-10 text-sm md:text-xs text-rose-600 font-mono text-center bg-white focus-visible:ring-rose-500"
                          />
                        </div>
                      </div>

                      <div className="flex items-center justify-between text-[11px] pt-1">
                        <span className="text-slate-500 font-medium">Hasil Stok Siap Jual:</span>
        ...
        {viewOnlyShipment && (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Rincian Barang ({viewOnlyShipment.items.length})</div>
              <div className="space-y-2 max-h-[45vh] overflow-y-auto pr-1">
                {viewOnlyShipment.items.map((item) => (
                  <div key={item.id} className="p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <div className="font-bold text-sm text-slate-900">{item.masterProduct.name}</div>
                        <div className="text-[11px] text-slate-500 mt-0.5">Varian: {item.masterProduct.variant}</div>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] uppercase text-slate-400 font-medium block">Dikirim HQ</span>
                        <span className="font-bold text-slate-900 text-sm">{item.qtySent} unit</span>
                      </div>
                    </div>
                    
                    <div className="flex justify-between items-center pt-3 border-t border-slate-100 text-xs bg-slate-50/50 -mx-4 px-4 pb-1">
                      <div className="space-y-0.5">
                        <span className="text-slate-500 block">Diterima Baik</span>
                        <span className="font-bold text-emerald-600 text-sm">{item.qtyReceived - item.qtyDamaged} unit</span>
                      </div>
                      <div className="text-right space-y-0.5">
                        <span className="text-slate-500 block">Rusak/Hilang</span>
                        <span className={`font-bold text-sm ${item.qtyDamaged > 0 ? 'text-rose-600' : 'text-slate-400'}`}>{item.qtyDamaged} unit</span>
                      </div>
                    </div>
                  </div>
                ))}
