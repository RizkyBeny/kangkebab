...
export const HQAnalyticsModule: React.FC<HQAnalyticsModuleProps> = ({ analytics, currentUser }) => {
  if (!analytics) {
    return (
      <Card className="p-8 text-center text-xs text-slate-400 border-dashed border-slate-200">
    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-500 mt-1.5 font-medium leading-relaxed max-w-xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="shadow-sm border-slate-200 rounded-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Omzet</CardTitle>
        ...
        <Card className="shadow-sm border-slate-200 rounded-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-bold text-slate-500 uppercase tracking-wider">Modal (COGS)</CardTitle>
        ...
        <Card className="shadow-sm border-slate-200 rounded-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-bold text-slate-500 uppercase tracking-wider">Gross Margin</CardTitle>
        ...
        <Card className="shadow-sm border-slate-200 rounded-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-bold text-slate-500 uppercase tracking-wider">Barang Rusak</CardTitle>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="shadow-sm border-slate-200 rounded-xl flex items-center justify-between p-5 flex-row">
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">Penjualan langsung di kasir toko fisik</p>
          </div>
        ...
        <Card className="shadow-sm border-slate-200 rounded-xl flex items-center justify-between p-5 flex-row">
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">Penjualan online via Shopee / TikTok Shop</p>
          </div>
        ...
        <div className="space-y-4">
            ...
            <Table>
              <TableHeader className="bg-slate-50/80 border-b border-slate-100">
                <TableRow className="hover:bg-transparent">
                  <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider py-4">Kode</TableHead>
                  <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Nama Cabang</TableHead>
                  ...
                  <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Margin Kotor</TableHead>
                  <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Transaksi</TableHead>
                  <TableHead className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Brg. Rusak</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {analytics.branchPerformance.map((b) => {
                  ...
                  return (
                    <TableRow key={b.branchId} className="group transition-colors hover:bg-slate-50/50">
                      <TableCell className="font-mono font-medium text-slate-600 text-xs py-4">{b.branchCode}</TableCell>
                      <TableCell className="font-bold text-slate-900 text-sm">{b.branchName}</TableCell>
                      <TableCell className="font-bold text-slate-900 font-mono text-sm">{formatRupiah(b.revenue)}</TableCell>
                      <TableCell className="text-slate-600 font-mono text-xs">{formatRupiah(b.cost)}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="font-semibold text-slate-700 text-xs">{b.transactionCount} tx</TableCell>
                      <TableCell>
                        <span className={`font-semibold text-xs ${b.damagedCount > 0 ? 'text-rose-600' : 'text-slate-400'}`}>
                          {b.damagedCount} unit
                        </span>
                      </TableCell>
                    </TableRow>
          ...
          <div className="md:hidden space-y-3">
            {analytics.branchPerformance.map((b) => {
              const marginPct = b.revenue > 0 ? (b.margin / b.revenue) * 100 : 0;
              return (
                <Card key={b.branchId} className="border-slate-200 shadow-sm rounded-xl">
                  <CardContent className="p-4 space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="font-bold text-slate-900 text-base">{b.branchName}</div>
                        <div className="text-[11px] text-slate-500 font-mono mt-0.5">Kode: {b.branchCode}</div>
                      </div>
                      <Badge variant="outline" className={`${b.damagedCount > 0 ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-slate-50 text-slate-500'} text-[10px]`}>
                    ...
                    <div className="bg-slate-50 rounded-lg p-3 space-y-2 border border-slate-100">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-slate-500 font-medium">Total Omzet</span>
                        <span className="font-mono text-slate-900 font-bold">{formatRupiah(b.revenue)}</span>
                      </div>
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-slate-500 font-medium">Modal (COGS)</span>
                        <span className="font-mono text-slate-600">{formatRupiah(b.cost)}</span>
                      </div>
                      <div className="pt-2 border-t border-slate-200/60 flex justify-between items-center text-xs">
                        <span className="text-slate-600 font-bold">Margin Kotor</span>
